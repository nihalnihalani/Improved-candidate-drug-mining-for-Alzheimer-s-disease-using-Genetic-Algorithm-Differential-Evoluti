...
class DrugDiscovery:
    ...
    def DifferentialEvolutionModel(self, regressor, instructions, numGenerations, fileW):
        ...
        for generation in range(numGenerations):
            ...
            trackR2test, testRMSE, testMAE, \
            testAccPred = self.evaluate_population(model=regressor, instructions=instructions, data=self.data,
                                                   population=population, exportfile=fileW)
            counter = 0
            dummy = []
            trail = []
            for i in range(50):
...
def main():
    ...
    file_path = os.path.join("/content/drive/My Drive/Masters/Data mining/Project3", output_filename)
    ...
    regressor = svm.SVR()
    instructions = {'dim_limit': 4, 'algorithm': 'DE', 'MLM_type': 'SVM'}
    Alzheimer.DifferentialEvolutionModel(regressor, instructions, 10000, fileW)
...
if __name__ == "__main__":
    main()
# Nihal Nihalani's Google Colab python notebook