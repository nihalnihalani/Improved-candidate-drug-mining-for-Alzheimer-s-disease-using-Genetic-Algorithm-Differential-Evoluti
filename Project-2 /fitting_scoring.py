    ...
    def evaluate_population(self, model, instructions, data, population, exportfile):
        ...
        for i in range(numOfPop):
            ...
            if fitness[i] < unfit:
                ...
                # store stats
                trackDesc[idx] = (re.sub(",", ",", str(xi)))  # Editing descriptor set to match actual indices in the original data.
                trackDesc[idx] = (re.sub(",", ",", str(UsedDesc[xi].tolist())))  # Editing descriptor set to match actual indices in the original data. the original data.
